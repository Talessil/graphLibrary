...
void fechoDireto(no *grafo, no *vertice, info *inf)
{
    ...
    buscaProfundidade(grafo,inf,fechoD,cont,1); //faz a busca em profundidade, passando a posição do vértice como a posição inicial da busca
...
void fechoIndireto(no *grafo, no *vertice, info *inf)
{
    ...
    buscaProfundidade(grafoTrans,inf,fechoD,cont,1); // faz a busca em profundidade do grafo, a partir da posição do respectivo nó
...
void compForteConexa(no *grafo, info *inf)
{
    ...
    int *vetp =(int*)malloc(sizeof(int)); // ponteiro auxiliar que contém valor referente a posição que cada nó está sendo visitado
    vetp[0] = 0;
    int k=0;
...
int proxComponente2(no *grafo, info *inf, int *visitados, int *novo)
{
    ...
    for(i=0; i<inf->numNo; i++)
    {
        ...
        if(visitados[k]==0 && novo[i]>p) // se nó não foi visitado e seu valor no vetor 'novo' for maior que o valor de 'p',
